...
theorem REInv.step {σ : Sigma} {L : Program} {H H' : Heap}
  ...
  : REInv σ L (.mk H' Γ' S' e') := by
    ...
    | methCall hHl hcls hbody => exact reinv_step_methCall hHl (hcls ▸ hbody) hinv hin
    | ret                 => exact reinv_step_ret hinv
    | newAlloc hHl _ _ _  => exact reinv_step_alloc hHl hi hinv
...
theorem access_implies_dep {L : Program} {G G': GlobName} {S cfs r : Stack}
